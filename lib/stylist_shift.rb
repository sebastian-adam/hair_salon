class StylistShift
  ...
  define_singleton_method(:all) do
    ...
    returned_stylist_shifts.each() do |stylist_shift|
      ...
      stylist_shifts.push(StylistShift.new({:id => id, :mon => mon, :tue => tue, :wed => wed, :thu => thu, :fri => fri, :stylist_id => stylist_id}))
    end
    stylist_shifts
  end
...
define_method(:save) do
  result = DB.exec("INSERT INTO stylist_shifts (mon, tue, wed, thu, fri, stylist_id) VALUES ('#{@mon}', '#{@tue}', '#{@wed}', '#{@thu}', '#{@fri}', #{@stylist_id}) RETURNING id;")
